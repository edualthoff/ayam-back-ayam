...
@Service
public class AccountLoginSocialMedia implements Serializable {
	private static final long serialVersionUID = 7877562880121481460L;

	private static final Logger log = LoggerFactory.getLogger(AccountLoginSocialMedia.class);
	...
	/**
	 * Valida o usuario vindo Dos meios de acesso - Facebook, google e etc no Sistema
	 * Caso não existir cadastra
	 * @param socialMediaNameEnum
	 * @param tokenMediaSocial
	 * @return
	 */
	// Refatorar a parde de autenticacao no sistema com senha "123" -  se o usuario redefinir senh por email vai da erro
	public Usuario login(SocialMediaNameEnum socialMediaNameEnum, String tokenMediaSocial) {
		SocialMediaModel socialMediaModel = socialMediaService.acessarInfoDoUsuario(socialMediaNameEnum, tokenMediaSocial);
		if(socialMediaModel != null) {
			SocialMediaAuth socialMediaAuth = socialMediaAuthService.findById(new SocialMediaAuthId(socialMediaNameEnum, socialMediaModel.getId()));
	...
	private SocialMediaAuth register(SocialMediaModel socialMediaModel, SocialMediaNameEnum socialMediaNameEnum) {
		...
		person.setNomeCompleto(socialMediaModel.getNome());
